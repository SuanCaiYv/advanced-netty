...
public class Msg {
    ...
    public byte[] asByteArrayLightweight() {
        return this.asStringLightweight().getBytes();
    }

    public String asStringLightweight() {
        long curr = System.nanoTime();
        // 作为head，body，tail之间的分隔符。
        String splitStr0 = String.format("%020d", curr);
        // 作为head内部，body内部，tail内部的分隔符。
        String splitStr1 = String.format("%020d", curr + 1);
        String splitStr2 = String.format("%020d", curr + 2);
        String splitStr3 = String.format("%020d", curr + 3);
        String head = this.head.asStringLightweight(splitStr1);
        String body = this.body.asStringLightweight(splitStr2);
        String tail = this.tail.asStringLightweight(splitStr3);
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder
                .append(head)
                .append(splitStr0)
    ...
    public static Msg withInit(long sender) {
        String initStr = "INIT";
        Msg msg = withEmpty();
        msg.getHead().setId(sender + "-0");
        msg.getHead().setSize(initStr.length());
        msg.getHead().setType(MsgType.INIT);
        msg.getHead().setSender(sender);
        msg.getHead().setReceiver(-1L);
        msg.getBody().setBody(initStr);
        return msg;
    }

    public static Msg withPlainText(String plainText) {
        Msg msg = withEmpty();
        msg.getHead().setId(UUID.randomUUID().toString());
        msg.getHead().setSize(plainText.length());
        msg.getHead().setType(MsgType.TEXT);
        msg.getHead().setSender(-1L);
        msg.getHead().setReceiver(-1L);
        msg.getBody().setBody(plainText);
        return msg;
    }

    public static Msg withError(Throwable throwable) {
        Msg msg = withEmpty();
        msg.getHead().setId(UUID.randomUUID().toString());
        msg.getHead().setSize(throwable.getMessage().length());
        msg.getHead().setType(MsgType.ERROR);
        msg.getHead().setSender(0L);
        msg.getHead().setReceiver(-1L);
        msg.getBody().setBody(throwable.getMessage());
        return msg;
    }
}
